...
volatile uint32_t* buffer_A_0 = (volatile uint32_t*)0x1a000;
volatile uint32_t* buffer_A_1 = (volatile uint32_t*)0x1b000;

volatile uint32_t* buffer_B_0 = (volatile uint32_t*)0x1c000;
volatile uint32_t* buffer_B_1 = (volatile uint32_t*)0x1d000;

void run_kernel()
{
    _llk_unpack_AB_hw_configure_<is_fp32_dest_acc_en, StochRndType::None>(DATA_FORMAT, DATA_FORMAT, DATA_FORMAT, DATA_FORMAT);
    _llk_unpack_AB_init_<>();
    _llk_unpack_AB_<>((std::uint32_t)buffer_A_0/16-1,(std::uint32_t)buffer_B_0/16-1);

    _llk_unpack_AB_hw_configure_<is_fp32_dest_acc_en, StochRndType::None>(DATA_FORMAT, DATA_FORMAT, DATA_FORMAT, DATA_FORMAT);
    _llk_unpack_AB_init_<>();
    _llk_unpack_AB_<>((std::uint32_t)buffer_A_1/16-1,(std::uint32_t)buffer_B_1/16-1);
}
...
volatile uint32_t* buffer_Dest_0 = (volatile uint32_t*)0x1e000;
volatile uint32_t* buffer_Dest_1 = (volatile uint32_t*)0x1f000;
void run_kernel()
{
    for(int i = 0; i < 16*16*4; i++)
    {
        buffer_Dest_0[i] = 0xdeadbeef;
    }
    ...
    _llk_pack_<DstSync::SyncFull,false, is_fp32_dest_acc_en>(0, (std::uint32_t)buffer_Dest_0/16-1);
    _llk_pack_dest_section_done_<DstSync::SyncFull,is_fp32_dest_acc_en>();


    for(int i = 0; i < 16*16*4; i++)
    {
        buffer_Dest_1[i] = 0xdeadbeef;
    }
    _llk_pack_hw_configure_<false, is_fp32_dest_acc_en, false>(DATA_FORMAT, DATA_FORMAT, 16*16*4);
    _llk_pack_init_<false, false, DstTileFaceLayout::RowMajor, false>(DATA_FORMAT);
    #ifdef ARCH_BLACKHOLE
    ...
    #endif
    _llk_packer_wait_for_math_done_();
    _llk_pack_<DstSync::SyncFull,false, is_fp32_dest_acc_en>(0, (std::uint32_t)buffer_Dest_1/16-1);
    _llk_pack_dest_section_done_<DstSync::SyncFull,is_fp32_dest_acc_en>();
}
