...
void run_kernel()
{
    // *((volatile uint32_t*)0x15200) = (uint32_t)is_fp32_dest_acc_en;
    ...
    _llk_unpack_A_init_<BroadcastType::NONE, false, EltwiseBinaryReuseDestType::NONE, unpack_to_dest>(0, 0, FACE_R_DIM, 4, IN_FORMAT,TF32);
    _llk_unpack_A_hw_configure_<is_fp32_dest_acc_en, StochRndType::None>(IN_FORMAT,TF32,FACE_R_DIM,0,4);
    _llk_unpack_A_<BroadcastType::NONE, false, EltwiseBinaryReuseDestType::NONE, unpack_to_dest>((((uint32_t)buffer_A)/16)-1, 0, IN_FORMAT,TF32);
...
{
    // copy srca to dest
    _llk_math_eltwise_unary_datacopy_init_<DataCopyType::A2D, BroadcastType::NONE,false, is_fp32_dest_acc_en, true>(0, 0, 4, IN_FORMAT);
    _llk_math_pack_sync_init_<DstSync::SyncFull,is_fp32_dest_acc_en>();
    _llk_math_hw_configure_<false,false>(IN_FORMAT, IN_FORMAT);
    _llk_math_wait_for_dest_available_<DstSync::SyncFull>();
    _llk_math_eltwise_unary_datacopy_<DataCopyType::A2D, DstSync::SyncFull, BroadcastType::NONE, is_fp32_dest_acc_en, unpack_to_dest>(0, TF32, IN_FORMAT);
